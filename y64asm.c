...
symbol_t* find_symbol(char *name)
{
	...
	while (stmp) {
		if (!strncmp(stmp->name, name, strlen(stmp->name)))
			return stmp;
		else stmp = stmp->next;
	}
    return NULL;
...
int add_reloc(char *name, bin_t *bin)
{
	...
	while (rtmp) {
		if (!strncmp(rtmp->name, name, strlen(rtmp->name)))
			return -1;
		else {
			rtmp = rtmp->next;
			reloc_tail = rtmp;
...
#define IS_HEXNUM(s) ((*(s)>='0' && *(s)<='9') || (*(s)>='a' && *(s)<='f') || (*(s)>='A' && *(s)<='F'))
...
parse_t parse_instr(char **ptr, instr_t **inst)
{
	...
	instr_t *instr= find_instr(ptr_cpy);
	if (instr == NULL) {
		return PARSE_ERR;
	}
...
parse_t parse_reg(char **ptr, regid_t *regid)
{
    ...
    /* find register */
	reg_t* regist = find_register(ptr_cpy);
	if (regist == NULL) {
		return PARSE_ERR;
	}
    /* set 'ptr' and 'regid' */
	(*regid) = regist->id;
	ptr_cpy += regist->namelen;
	ptr = &ptr_cpy;
    return PARSE_REG;
}
...
parse_t parse_symbol(char **ptr, char **name)
{
	...
	if (!(IS_LETTER(ptr_cpy) || ((*ptr_cpy) == '_'))) {
		return PARSE_ERR;
	}
	char* name_start = ptr_cpy;

	while ((!IS_END(ptr_cpy)) &&
		(IS_LETTER(ptr_cpy) || ((*ptr_cpy) == '_') || IS_NUM(ptr_cpy))){
		ptr_cpy++;
	}
	char* name_end = ptr_cpy;


    /* allocate name and copy to it */
	int name_len = name_end - name_start;
	char* sym_name = (char*)malloc((name_len + 1) * sizeof(char));
...
parse_t parse_digit(char **ptr, long *value)
{
	...
	radix_t radix;
	/* if the num is in hex*/
	if (((*ptr_cpy) == '0') && ((*(ptr_cpy + 1) == 'x') || (*(ptr_cpy + 1) == 'X'))) {
		ptr_cpy += 2;
		while (IS_HEXNUM(ptr_cpy)) {
			ptr_cpy++;
		}
		radix = HEX;
	}
	/* if the num is in dec*/
	else{
		while (IS_NUM(ptr_cpy)) {
			ptr_cpy++;
		}
		radix = DEC;
	}

	char* digit_end = ptr_cpy;

    /* calculate the digit, (NOTE: see strtoull()) */
	long val;

	if (radix == HEX) {
		val = strtoull(digit_start, NULL, 16);
	}
	else{
		val = strtoull(digit_start, NULL, 10);
	}
...
parse_t parse_imm(char **ptr, char **name, long *value)
{
	...
	else if (IS_LETTER(ptr_cpy) || ((*ptr_cpy) == '_')) {
		type = 1;
	}
	else {
		return PARSE_ERR;
	...
	if (type == 0) {
		char** ptr_cpy_p = &ptr_cpy;
		if (parse_digit(ptr_cpy_p, value) == PARSE_ERR) {
			return PARSE_ERR;
		}
		ptr = ptr_cpy_p;
		return PARSE_DIGIT;
    ...
    /* if IS_LETTER, then parse the symbol */
		...
		char** ptr_cpy_p = &ptr_cpy;
		if (parse_symbol(ptr_cpy_p, name) == PARSE_ERR) {
			return PARSE_ERR;
		}
		ptr = ptr_cpy_p;
		return PARSE_SYMBOL;
...
parse_t parse_mem(char **ptr, long *value, regid_t *regid)
{
	...
	char** ptr_cpy_p = &ptr_cpy;
	if (parse_reg(ptr_cpy_p, regid) == PARSE_ERR) {
		return PARSE_ERR;
	}
	ptr_cpy = *ptr_cpy_p;
